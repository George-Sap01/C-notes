...
//θα διαβαζω χαρακτηρες απο το file και μετα θα πηγαινω στο struct και θα αυξηνω κατα 1
...
int add_hash_table(ptrs *hash_table, char c){
    ...
    {    
        hash_table[index].root = newnode; //βαζω την ριζα να δειχνει στο newnode αφου ειναι αδεια
        return TRUE;
    }
}
...
int BST_insert(character *oldnode, character *newnode)
{
    // εδω ξερουμε πως το root δειχνει καπου, το oldnode ειναι το struct που δειχνει το root
    // τωρα θελουμε να το τοποθετησουμε αναλογα, δεξια ή αριστερα 

    character *cur = oldnode;
    while(1)
    {
        if( newnode->c[0] > cur->c[0]) // δεξια
        {
            if( cur->right == NULL)
            {
                cur->right = newnode;
