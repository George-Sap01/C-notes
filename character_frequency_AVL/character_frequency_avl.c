/*  -------------------     Character Frequency Counter (AVL Tree in C)     -------------------
    Notes:

    **** How It Works
    1. Pre-builds three balanced AVL trees containing target characters.
    2. Reads `words.txt` character-by-character.
    3. Traverses the appropriate tree to increment character frequencies.
    4. Outputs individual character counts, total character sum, and execution time.


    i was curious to see if i could implement an AVL Tree.
...
int main()
{
    ...
    // initialize of the 3 AVL Trees
    TR_init(&lower);
    TR_init(&upper);
    TR_init(&symbols);

    // initialize the symbol arrays 
    for(i=0; i<26; i++){    
        array_lower[i] = i + 97;
        array_upper[i] = i + 65;}

    // inserting the arrays in the AVL Trees
    for(i=0; i<26; i++){    
    ...
    while(1)
    {
        ...
        if(c >= 65 && c<= 90) // big characters
            TR_UPDATE(&upper, c);
        else if (c>=97 && c<=122) // small characters
            TR_UPDATE(&lower, c);
        else if(c==32 || c==33 || c==44 || c==46 || c==63 || c==34 || c == 45 || c==58 || c==59) // symbols
            TR_UPDATE(&symbols, c);
        else
            continue;
    }
