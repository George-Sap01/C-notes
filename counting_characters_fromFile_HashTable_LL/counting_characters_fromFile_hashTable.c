/*
    ...
    θα τα περασω στην δομη δεδομενων πριν ξεκινησει η διαπεραση στο αρχειο.
*/
...
struct CHARACTER{
    char c[2]; // ο χαρακτηρας μας 
    int cnt; // ποσες φορες εμφανιζεται
...
int hash_func(char c); //θα διαβαζω χαρακτηρες απο το file και μετα θα πηγαινω στο struct και θα αυξηνω κατα 1
...
// add in linked list, i need the pointers(start, end) and the node 
int ll_add(ptrs *p, character *node);
// update counter
...
int main()
{
    ...
    /*   αρχικοποιηση των πινακων με τους χαρακτηρες    */
    for(i=0; i<26; i++){    
        array_lower[i] = i + 97;
        array_upper[i] = i + 65;}
    ...
    /*   περναω τους χαρακτηρες στο HASH TABLE   */
    for(i=0; i<26; i++){    
...
int add_hash_table(ptrs *hash_table, char c){
    ...
    if(hash_table[index].start != NULL)
        return ll_add(&(hash_table[index]), newnode);
    else
    {    
...
void print_table(ptrs *hash_table){
    ...
    {    
        ...
        {   
            printf("\t%i ", i);
            character *current = hash_table[i].start;
            while(current != NULL)
            {
                printf("|Character: '%s', cnt: %d| -> ", current->c, current->cnt);
